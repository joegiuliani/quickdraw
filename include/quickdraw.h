...
// - Passed to mouse listeners on callback when a new mouse/cursor
//   event is polled
// - Contains mouse data that triggered the callback. All other data are given
//   as their last polled states
// - Semantics: 'pressed' refers to the change in state of a button from UP
//   to DOWN, 'released' refers to the change in state of a button from DOWN to UP,
//   and 'down' refers to the whether the state of a button is DOWN.
// - is_down, is_pressed, and is_released are to be indexed with a MouseButton
//   such that, for example, is_down[MouseButton::LEFT] == true if the left
//   left mouse button's last reported state is down
...
void DrawPath(const std::vector<Vec2>& points, float thickness, const Vec2& offset)
{
    ...
    // This method draws connected quads as edges along a path
